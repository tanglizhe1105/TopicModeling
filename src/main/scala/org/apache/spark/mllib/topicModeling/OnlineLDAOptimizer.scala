...
final class OnlineLDAOptimizer extends LDAOptimizer with Serializable{
  ...
  def initialize(corpusSize: Long, windowSize: Int, vocabSize: Int, lda: LDA): this.type ={
    this.k = lda.getK
  ...
  def next(batch: RDD[(Long, Vector)]): RDD[(Long, BDV[Double])] = {
    submitMiniBatch(batch)
  }
   ...
   * subset.
   */
  private[topicModeling] def submitMiniBatch(batch: RDD[(Long, Vector)]): RDD[(Long, BDV[Double])] = {
    if (batch.isEmpty()) return null
    iteration += 1

    val (expElogbeta, stats, gammaArray) =
  ...
  def perplexity(docs: RDD[(Long, Vector)], gammaArray: RDD[(Long, BDV[Double])]): (Double, Double) = {
    val alphaVector = Vectors.dense(Array.fill(k)(alpha))
    val brzAlpha = alphaVector.toBreeze.toDenseVector
